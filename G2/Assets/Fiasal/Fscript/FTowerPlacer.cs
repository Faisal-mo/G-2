...
public class FTowerPlacer : MonoBehaviour
{
    ...
    void HandlePlayer1Inputs()
    {
        ...
        {
            if (isUpgradingP1)
                UpgradeRandomTower(1);
            else if (isSendingBossP1)
                SendMiniBoss(1);
            else if (selectedTowerIndexP1 != -1)
                PlaceTower(1, towerPrefabsP1[selectedTowerIndexP1]);
        }
    }
    ...
    void HandlePlayer2Inputs()
    {
        ...
        {
            if (isUpgradingP2)
                UpgradeRandomTower(2);
            else if (isSendingBossP2)
                SendMiniBoss(2);
            else if (selectedTowerIndexP2 != -1)
                PlaceTower(2, towerPrefabsP2[selectedTowerIndexP2]);
        }
    }
    ...
    void UpgradeRandomTower(int playerID)
    {
        ...
        System.Collections.Generic.List<Tower> playerTowers = new System.Collections.Generic.List<Tower>();
        ...
        {
            var randomTower = playerTowers[UnityEngine.Random.Range(0, playerTowers.Count)];
            randomTower.Upgrade();
        }
        ResetPlayerState(playerID);
    }
